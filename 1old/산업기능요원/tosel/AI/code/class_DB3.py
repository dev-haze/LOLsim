...
class DB_search:
    ...
    def get_receiptNo_by_name(self, email, exam_no):#이름 : 작은따옴표
        base_query = "SELECT receipt_no, exam_no FROM receipt_2 where email = %s and exam_no = %s;"
        self.cur.execute(base_query,(email,exam_no))
        result = self.cur.fetchall()
        return result

    def get_data_by_name(self, email, exam_no):
...
print(db.get_receiptNo_by_name('asf@naver.com','44'))
